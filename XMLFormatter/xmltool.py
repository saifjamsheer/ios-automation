...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep 24 11:26:32 2018
...
def main(args, text_file_name, source_path, dest_path):
    if args.check:
        cprint(f2.renderText('CHECKING TEXT FILE\n'), "yellow")
        string_checker(text_file_name, source_path)
    elif args.format:
        cprint(f2.renderText('ATTEMPTING TO CONSTRUCT XML\n'), "cyan")
        xml_formatter(text_file_name, source_path, dest_path)
    else:
...
def string_checker(filename, source_path):
    ...
    pattern1 = re.compile("^\"[a-z0-9_]*\" \= \"\S.*\S(?=\")\"\;")
    pattern2 = re.compile("^\"[a-z0-9_]*\" \= \"\S(?=\")\"\;")
...
def xml_formatter(filename, source_path, dest_path):
    ...
    for elem in resources.iter(code):
...
if __name__ == '__main__':
    ...
    filename = "copy_doc.txt"
    source_path = "../" 
    dest_path = "../../../app/src/main/res/values"
    main(arguments, filename, source_path, dest_path)      
